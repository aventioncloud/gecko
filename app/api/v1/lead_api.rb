  ...
  class LeadAPI < Base
      ...
      get '/', authorize: ['read', 'Lead'] do
        ...
        #Retorna todos os leads do grupo, para responsável
          ...
          leadcount = Lead.joins(:user).joins(:contact).joins(:leadstatus).where("users.groups_id IN (?)", leads_group).count
          pages = (leadcount / per_page).ceil
          @lead = Lead.joins(:user).joins(:contact).joins(:leadstatus).select("users.groups_id, users.name as usuario, leads.*, contacts.id as contact_id, contacts.name, contacts.typecontact as tipo, lead_statuses.id as status_id, lead_statuses.name as status, to_char(leads.updated_at + INTERVAL '1 HOURS', 'DD/MM/YY HH24:MI') as data, '"+pages.to_s+"' as pages").where("users.groups_id IN (?) and ?", leads_group).paginate(:page => params[:page], :per_page => per_page).order(params[:orderby])
        #Retorna os Leads para usuário
        else
          leadcount = Lead.joins(:user).joins(:contact).joins(:leadstatus).where("leads.user_id = ? and ?",@user["id"]).count
          pages = (leadcount / per_page).ceil
